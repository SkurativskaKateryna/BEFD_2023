...
d <- ts(cbind(subset(cluster_means, Cluster == 1)$Value, subset(cluster_means_co2, Cluster == 1)$Value), start=2000, frequency=1)
colnames(d) <- c("Demand", "CO2")

as.data.frame(d) |>
  ggplot(aes(x=Demand, y=CO2)) +
  geom_point() +
  ylab("Demand (GW)") + xlab("CO2")

#Cluster 2

x <- ts(subset(cluster_means, Cluster == 2)$Value , start=2000, frequency = 1)
y <- ts(subset(cluster_means_co2, Cluster == 2)$Value, start=2000, frequency = 1)

d <- ts(cbind(subset(cluster_means, Cluster == 2)$Value, subset(cluster_means_co2, Cluster == 2)$Value), start=2000, frequency=1)
colnames(d) <- c("Demand", "CO2")
...
x <- ts(subset(cluster_means, Cluster == 3)$Value , start=2000, frequency = 1)
y <- ts(subset(cluster_means_co2, Cluster == 3)$Value, start=2000, frequency = 1)

d <- ts(cbind(subset(cluster_means, Cluster == 3)$Value, subset(cluster_means_co2, Cluster == 3)$Value), start=2000, frequency=1)
colnames(d) <- c("Demand", "CO2")
...
#Cluster 4
x <- ts(subset(cluster_means, Cluster == 4)$Value , start=2000, frequency = 1)
y <- ts(subset(cluster_means_co2, Cluster == 4)$Value, start=2000, frequency = 1)

d <- ts(cbind(subset(cluster_means, Cluster == 4)$Value, subset(cluster_means_co2, Cluster == 4)$Value), start=2000, frequency=1)
colnames(d) <- c("Demand", "CO2")
...
p1 <- ggplot(df, aes(x=Demand, y=CO2)) +
  geom_point()
...
```{r}
fit.demand<- tslm(x ~ trend)
summary(fit.demand)
checkresiduals(fit.demand)

autoplot(x, series="Data") +
  autolayer(fitted(fit.demand), series="Fitted") +
  xlab("Year") + ylab("co2") +
  ggtitle("demand co2")
